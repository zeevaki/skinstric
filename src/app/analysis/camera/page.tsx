...
export default function CameraPage() {
  ...
  const doCapture = () => {
    ...
    try { sessionStorage.setItem("skinstric_image", canvas.toDataURL("image/jpeg", 0.8)); } catch { /* too large */ }
    streamRef.current?.getTracks().forEach((t) => t.stop());
    setCaptured(true);
    setTimeout(() => router.push("/analysis/processing"), 1200);
  };
    ...
    return (
      <main className="relative h-screen w-full flex flex-col items-center justify-center" style={{ background: "#FCFCFC" }}>
        ...
        <div style={{ position: "relative", width: "min(740px, 80vmin)", height: "min(740px, 80vmin)", flexShrink: 0 }}>
          ...
          {/* "SETTING UP CAMERA ..." overlaid just below the icon center */}
          <p style={{ position: "absolute", left: "50%", top: "62%", transform: "translateX(-50%)", fontSize: 10, fontWeight: 600, letterSpacing: "-0.02em", textTransform: "uppercase", color: "#1A1B1C", whiteSpace: "nowrap" }}>
            {denied ? "Camera access denied — check browser settings." : "Setting up camera ..."}
          </p>
        </div>
  ...
  return (
    <main className="relative h-screen w-full overflow-hidden" style={{ background: "#1A1B1C" }}>
      ...
      {/* Video */}
      <video ref={videoRef} autoPlay playsInline muted className="absolute inset-0 w-full h-full object-cover" style={{ opacity: 0.92 }} />
      ...
      <div style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)", zIndex: 2 }}>
        ...
        {captured ? (
          /* GREAT SHOT! — after capture */
          <svg width="86" height="24" viewBox="0 0 86 24" fill="none" style={{ position: "absolute", top: "22%", left: "50%", transform: "translateX(-50%)" }}>
            <path d="M5.44434 18.112C6.88634 18.112 8.10434 17.37 8.56634 16.292V18H9.67234V14.64C9.67234 13.52 8.65034 12.96 7.93634 12.96H5.51434V13.996H7.72634C8.20234 13.996 8.49634 14.29 8.49634 14.738C8.49634 16.152 7.06834 17.062 5.45834 17.062C3.44234 17.062 1.95834 15.634 1.95834 13.1C1.95834 10.552 3.44234 9.166 5.45834 9.166C6.97034 9.166 8.20234 10.048 8.42634 11.294H9.68634C9.46234 9.418 7.71234 8.088 5.47234 8.088C3.03634 8.088 0.740344 9.754 0.740344 13.1C0.740344 16.446 3.02234 18.112 5.44434 18.112ZM16.7069 13.352C17.6309 12.918 18.2469 12.148 18.2469 11.07C18.2469 9.362 17.0569 8.2 15.1249 8.2H11.4149V18H12.6049V14.01H15.2649C16.1889 14.01 16.8049 14.682 16.8609 15.634L17.0289 18H18.1769L18.0089 15.564C17.9389 14.57 17.4769 13.772 16.7069 13.352ZM15.1249 9.264C16.4969 9.264 17.0149 10.104 17.0149 11.07C17.0149 12.036 16.4969 12.932 15.1249 12.932H12.6049V9.264H15.1249ZM26.7517 18V16.922H21.6557V13.478H25.6737V12.4H21.6557V9.278H26.5977V8.2H20.4657V18H26.7517ZM36.3474 18L32.4414 8.2H31.3634L27.4574 18H28.7034L29.7254 15.354H34.0934L35.1014 18H36.3474ZM31.9094 9.67L33.6734 14.29H30.1314L31.9094 9.67ZM39.6193 18V9.278H42.7413V8.2H35.2793V9.278H38.4153V18H39.6193ZM50.4542 18.112C52.4982 18.112 54.0382 17.104 54.0382 15.326C54.0382 11.644 48.2702 13.198 48.2702 10.692C48.2702 9.754 49.0542 9.166 50.3842 9.166C51.7002 9.166 52.5542 9.88 52.6662 10.986H53.8422C53.7022 9.222 52.3442 8.088 50.3842 8.088C48.4242 8.088 47.0662 9.138 47.0662 10.692C47.0662 14.192 52.7922 12.806 52.7922 15.326C52.7922 16.516 51.8122 17.034 50.4542 17.034C48.9282 17.034 48.0322 16.152 47.9062 14.794H46.7302C46.8562 16.796 48.2562 18.112 50.4542 18.112ZM61.9727 12.4H56.7787V8.2H55.5887V18H56.7787V13.478H61.9727V18H63.1627V8.2H61.9727V12.4ZM69.5951 18.112C72.3531 18.112 74.4111 15.984 74.4111 13.1C74.4111 10.216 72.3531 8.088 69.5951 8.088C66.8371 8.088 64.7931 10.216 64.7931 13.1C64.7931 15.984 66.8371 18.112 69.5951 18.112ZM69.5951 17.034C67.5371 17.034 66.0251 15.354 66.0251 13.1C66.0251 10.846 67.5371 9.166 69.5951 9.166C71.6531 9.166 73.1791 10.846 73.1791 13.1C73.1791 15.354 71.6531 17.034 69.5951 17.034ZM78.8029 18V9.278H81.9249V8.2H74.4629V9.278H77.5989V18H78.8029ZM84.3699 15.186H83.5719L83.2219 8.004H84.7199L84.3699 15.186ZM83.0959 17.244C83.0959 16.8987 83.1799 16.656 83.3479 16.516C83.5159 16.376 83.7212 16.306 83.9639 16.306C84.1972 16.306 84.3979 16.376 84.5659 16.516C84.7432 16.656 84.8319 16.8987 84.8319 17.244C84.8319 17.58 84.7432 17.8227 84.5659 17.972C84.3979 18.1213 84.1972 18.196 83.9639 18.196C83.7212 18.196 83.5159 18.1213 83.3479 17.972C83.1799 17.8227 83.0959 17.58 83.0959 17.244Z" fill="#FCFCFC"/>
          </svg>
        ) : countdown === null && (
          /* PLACE YOUR HEAD IN AN ELLIPSE — before countdown */
      ...
      {/* Countdown display — top of oval */}
      {countdown !== null && (
        <div className="absolute z-10 flex flex-col items-center gap-2" style={{ left: "50%", top: "20%", transform: "translateX(-50%)" }}>
          {timer === "3S" ? (
            /* 3S: show 1 2 3 with current highlighted */
            <div style={{ display: "flex", alignItems: "baseline", gap: "clamp(8px, 1.5vw, 24px)" }}>
              {[1, 2, 3].map((n) => (
                <span key={n} style={{ fontSize: n === countdown ? "clamp(3rem, 5vw, 5rem)" : "clamp(1.25rem, 2.5vw, 2.5rem)", fontWeight: 200, color: "#FCFCFC", opacity: n === countdown ? 0.85 : 0.3, transition: "all 0.25s ease", lineHeight: 1 }}>
                  {n}
                </span>
              ))}
            </div>
          ) : (
            /* 10S: show current number large */
            <span style={{ fontSize: "clamp(4rem, 7vw, 7rem)", fontWeight: 200, color: "#FCFCFC", opacity: 0.85, lineHeight: 1 }}>
              {countdown}
            </span>
          )}
          <svg width="75" height="24" viewBox="0 0 75 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M7.81453 12.4H2.62053V8.2H1.43053V18H2.62053V13.478H7.81453V18H9.00453V8.2H7.81453V12.4ZM15.4369 18.112C18.1949 18.112 20.2529 15.984 20.2529 13.1C20.2529 10.216 18.1949 8.088 15.4369 8.088C12.6789 8.088 10.6349 10.216 10.6349 13.1C10.6349 15.984 12.6789 18.112 15.4369 18.112ZM15.4369 17.034C13.3789 17.034 11.8669 15.354 11.8669 13.1C11.8669 10.846 13.3789 9.166 15.4369 9.166C17.4949 9.166 19.0209 10.846 19.0209 13.1C19.0209 15.354 17.4949 17.034 15.4369 17.034ZM27.9463 18V16.922H23.0463V8.2H21.8563V18H27.9463ZM32.0684 18C34.8124 18 36.9544 16.222 36.9544 13.1C36.9544 9.978 34.8124 8.2 32.0684 8.2H29.1844V18H32.0684ZM30.3744 9.278H32.0404C33.9444 9.278 35.7084 10.398 35.7084 13.1C35.7084 15.802 33.9444 16.922 32.0404 16.922H30.3744V9.278ZM45.091 18.112C47.135 18.112 48.675 17.104 48.675 15.326C48.675 11.644 42.907 13.198 42.907 10.692C42.907 9.754 43.691 9.166 45.021 9.166C46.337 9.166 47.191 9.88 47.303 10.986H48.479C48.339 9.222 46.981 8.088 45.021 8.088C43.061 8.088 41.703 9.138 41.703 10.692C41.703 14.192 47.429 12.806 47.429 15.326C47.429 16.516 46.449 17.034 45.091 17.034C43.565 17.034 42.669 16.152 42.543 14.794H41.367C41.493 16.796 42.893 18.112 45.091 18.112ZM53.4377 18V9.278H56.5597V8.2H49.0977V9.278H52.2337V18H53.4377ZM58.9076 18V8.2H57.7176V18H58.9076ZM67.0752 18V16.922H62.1752V8.2H60.9852V18H67.0752ZM74.4033 18V16.922H69.5033V8.2H68.3133V18H74.4033Z" fill="#FCFCFC"/>
          </svg>
        </div>
      )}
      ...
      <div className="absolute z-[3] flex items-center gap-3 select-none" style={{ left: 32, top: "50%", transform: "translateY(-50%)" }}>
        ...
        {TIMER_OPTIONS.map((opt) => (
          <span
            ...
            style={{ fontSize: 10, fontWeight: 600, letterSpacing: "-0.02em", textTransform: "uppercase", color: "#FCFCFC", opacity: timer === opt ? 1 : 0.4, cursor: "pointer" }}
          >
            {opt}
          </span>
        ))}
      ...
      <div
        ...
        style={{ right: 32, top: "50%", transform: "translateY(-50%)", opacity: countdown !== null ? 0.4 : 1 }}
      >
        <TakePictureButton />
      </div>
